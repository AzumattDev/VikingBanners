...
namespace VikingBanners
{
    ...
    public class VikingBannersPlugin : BaseUnityPlugin
    {
        ...
        public void Awake()
        {
            ...
            useServerBannerURL = config("1 - General", "Use Server Banner URL", Toggle.Off, new ConfigDescription("A toggle that when turned on, sets all ship banners to use the Server Banner URL.", null, new ConfigurationManagerAttributes { Order = 5 }));
            ...
            keyConfig = config("1 - General", "Edit Key", new KeyboardShortcut(KeyCode.Mouse1), new ConfigDescription("A keyboard shortcut that allows the player to interact with the ship to change the banner image", null, new ConfigurationManagerAttributes { Order = 3 }), false);
            requireKeyConfig = config("1 - General", "Require Key Press", Toggle.Off, new ConfigDescription($"A toggle that when turned on, requires the player to hold down the `Edit Key` <{keyConfig.Value.ToString()}> in order to interact with the ship to change the banners.", null, new ConfigurationManagerAttributes { Order = 2 }));
