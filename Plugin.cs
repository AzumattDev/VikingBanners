...
namespace VikingBanners
{
    ...
    public class VikingBannersPlugin : BaseUnityPlugin
    {
        ...
        internal static VikingBannersPlugin instance;

        public static readonly ManualLogSource VikingBannersLogger =
        ...
        public void Awake()
        {
            ...
            useServerBannerURL = config("1 - General", "Use Server Banner URL", Toggle.Off, new ConfigDescription("A toggle that when turned on, sets all banners to use the Server Banner URL.", null, new ConfigurationManagerAttributes { Order = 5 }));
            serverBannerURL = TextEntryConfig("1 - General", "Server Banner URL", "https://i.imgur.com/TbcJ3LU.png", new ConfigDescription("Put a valid image URL here, this entry field should contain the URL of the image to use for the banners when the `Use Server Banner URL` toggle is on.", null, new ConfigurationManagerAttributes { Order = 4 }));
            keyConfig = config("1 - General", "Edit Key", new KeyboardShortcut(KeyCode.Mouse1), new ConfigDescription("A keyboard shortcut that allows the player to interact with the banner to change the banner image", null, new ConfigurationManagerAttributes { Order = 3 }), false);
            requireKeyConfig = config("1 - General", "Require Key Press", Toggle.Off, new ConfigDescription($"A toggle that when turned on, requires the player to hold down the `Edit Key` <{keyConfig.Value.ToString()}> in order to interact with the banner to change the banners texture.", null, new ConfigurationManagerAttributes { Order = 2 }));
        ...
        private void ReadConfigValues(object sender, FileSystemEventArgs e)
        {
            ...
            try
            {
                VikingBannersLogger.LogDebug("ReadConfigValues called");
                Config.Reload();
            }
            catch
            {
                VikingBannersLogger.LogError($"There was an issue loading your {ConfigFileName}");
                VikingBannersLogger.LogError("Please check your config entries for spelling and format!");
            }
        }
        ...
        internal static ConfigEntry<Toggle> useServerBannerURL = null!;
        internal static ConfigEntry<string> serverBannerURL = null!;
